...
1. User-facing Devpost task:
   - return to the Devpost draft;
   - continue from Project Overview with `Save & continue`;
   - capture/send the next screen so every field can be optimized for judging.
2. Finish FOD-A source split discovery; the workflow now records candidate train/validation/test/ImageSets files as a provenance artifact.
...
- Prior clean OpenCV 5 CI reference: `https://github.com/pmartins87/OpenCV/actions/runs/32980264697`
- Successful FOD-A provenance run: `https://github.com/pmartins87/OpenCV/actions/runs/32994883456`
